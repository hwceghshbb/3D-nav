...
namespace scan_planner
{
  ...
  bool SCANPlannerManager::checkTrajectoryCollisionFree(UniformBspline position_traj)
  {
    ...
    const double length = (position_traj.evaluateDeBoorT(duration) - position_traj.evaluateDeBoorT(0.0)).norm();
    const double spatial_samples = std::max(1.0, length / std::max(grid_map_->getResolution(), 1e-3));
    const double sample_dt = std::max(0.01, duration / spatial_samples);
