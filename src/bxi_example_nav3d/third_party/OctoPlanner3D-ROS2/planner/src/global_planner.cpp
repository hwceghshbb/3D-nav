...
namespace global_planner
{
    ...
    bool GlobalPlanner::startPlan()
    {
        ...
        while (!open_set.empty() && iters < max_iterations) 
        {
            ...
            for (const auto & d : directions) 
            {
                ...
                // support and preblocked-cell checks to every map voxel. Reusing
        ...
        const GridIndex & seed, int radius_cells, int component_id, GridIndex & out) const
    {
        ...
        for (int radius = 1; radius <= std::max(0, radius_cells); ++radius) {
            for (int dz = 0; dz <= radius; ++dz) {
                for (int dx = -radius; dx <= radius; ++dx) {
                    for (int dy = -radius; dy <= radius; ++dy) {
                        ...
                        const GridIndex above{seed.x + dx, seed.y + dy, seed.z + dz};
                        if (belongs_to_component(above)) {
                            out = above;
                            return true;
                        }
                        if (dz > 0) {
                            const GridIndex below{seed.x + dx, seed.y + dy, seed.z - dz};
                            if (belongs_to_component(below)) {
                                out = below;
                                return true;
                            }
                        }
