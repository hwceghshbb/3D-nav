            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                if(mpAtlas->KeyFramesInMap()>2)
                {
                    ...
                    if(mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized())
                    {
                        ...
                        const float init_motion_threshold = mpTracker->mFastInit ? 0.02f : 0.05f;
                        if(dist>init_motion_threshold)
                            mTinit += mpCurrentKeyFrame->mTimeStamp - mpCurrentKeyFrame->mPrevKF->mTimeStamp;
                        if(!mpCurrentKeyFrame->GetMap()->GetIniertialBA2())
                        {
                            if((mTinit<10.f) && (dist<0.02) && !mpTracker->mFastInit)
                            {
                            ...
                            else if((mTinit<10.f) && (dist<0.02))
                            {
                                cout << "Not enough motion for initializing. Keep moving slowly; fast IMU init keeps the active map." << endl;
                            }
                        }
