...
namespace scan_planner
{
  namespace
  {
    double collisionCheckTimeStep(UniformBspline &traj, const GridMap::Ptr &grid_map)
    {
      ...
      const double duration = std::max(t_max - t_min, 1e-3);
      const double length = (traj.evaluateDeBoorT(t_max) - traj.evaluateDeBoorT(t_min)).norm();
      const double spatial_samples = std::max(1.0, length / std::max(grid_map->getResolution(), 1e-3));
      return std::max(0.01, duration / spatial_samples);
    }
  } // namespace
